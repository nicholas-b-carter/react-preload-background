...
export default class imageBackgroundPreloader extends React.Component {
  ...
  render() {
    const {src, placeholder, className = this.state.loaded ? ' image image-loaded ': ' image', children = null, ...props} = this.props;
    const source = !this.state.loaded || this.state.error ? placeholder : src;
    return (
      <div style={{PreloadBackground: `url(${source})`}} {...props} className={className}>
        {children}
      </div>
    );
  }
